...
try {
    ...
    if (typeof e.code !== 'undefined' && e.code === 'MODULE_NOT_FOUND') {
        error = "'config.json' 을 찾지 못했습니다! \n먼저 remote주소로 설정을 하시는게 어떠하신지...?"
    } else if (typeof e.message !== 'undefined') {
        console.log(e)
        error = "구문 오류. \n니꺼 config.json에 오류가 있는거 같은데... : " + e.message + '\n' +
            '팁 : JavaScript설정을 여기 http://jshint.com/ 서 받아서 설정하는것도 도움이 될 수 있어.'
    }
    console.log(error)
}
...
if (config.remote && config.remote.enabled || firstRun) {
    ...
    console.log('원격 제어 주소는 http://%s:%d 입니다.', addresses[0], config.remote.port)
